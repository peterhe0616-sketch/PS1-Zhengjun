...
\textbf{Result.} At $F=50$, $p^{*}=0.5000$; the required $p$ falls as $F$ rises ($F=100\Rightarrow0.25$; $F=400\Rightarrow0.0625$), and below $F=25$ no $p\le1$ suffices. All results are \emph{simulated}; the Colab notebook reproduces them.
...
\textbf{Consequence and limits.} If licensing dominates, verification must not be a binary pass: it should report which claims were recomputed. A simulated table cannot establish human behavior; the next test crosses trace-file presence with check depth.
